...
// 2. 创建引擎
ICudaEngine* createEngine(const std::string& onnx_file, float addValue)
{
    // 2.1 builder
    ...
    PluginFieldCollection pluginFieldCollection{1,&valueField};
    IPluginV2* addPlugin =  pluginCreator->createPlugin("add_constant_plugin", &pluginFieldCollection);
    
    // 拿取softmax的输出
    ITensor* softmaxOutput = network->getOutput(0);
    network->unmarkOutput(*softmaxOutput);
    softmaxOutput ->setName("softmax_output");

    
    ITensor* pluginInputs[] = {softmaxOutput};
    ILayer* pluginLayer = network->addPluginV2(pluginInputs, 1, *addPlugin);
    pluginLayer->setName("plugin_layer");
    ITensor* pluginOutput = pluginLayer->getOutput(0);
    pluginOutput->setName("output");
    network->markOutput(*pluginOutput);
...
int main()
{
    ...
    std::string engine_path = "tensorrt-8.6/04-mnist-plugin-onnx01/artifacts/lenet.engine";
    std::string input_path = "tensorrt-8.6/04-mnist-plugin-onnx01/artifacts/input.bin";
    // 注意：Python 端保存的 output_pytorch.bin 已过 softmax，直接是概率
    std::string pytorch_output_path = "tensorrt-8.6/04-mnist-plugin-onnx01/artifacts/output_pytorch.bin";
    std::string onnx_file = "tensorrt-8.6/04-mnist-plugin-onnx01/artifacts/lenet.onnx";

    // 加载plugin
    constexpr float kAddValue = 3.0F;
    if(!registerAddConstantPlugin()){
        std::cerr << "Failed to register add constant plugin" << std::endl;
        return 1;
    }
    ...
    if (fileExists(engine_path)) {
        ...
        engine = createEngine(onnx_file,kAddValue);
        assert(engine);
