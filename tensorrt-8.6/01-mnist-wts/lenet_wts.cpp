...
// 2. 定义lenet的网络
ICudaEngine* buildLenetEngine(IBuilder* builder, IBuilderConfig* config, const std::map<std::string, Weights>& weightMap)
{
    // 1. 创建network
...
int main(int argc, char** argv)
{
    ...
    std::string wts_path = "tensorrt-8.6/01-mnist-wts/artifacts/lenet.wts";
    std::string engine_path = "tensorrt-8.6/01-mnist-wts/artifacts/lenet.engine";
    std::string input_path = "tensorrt-8.6/01-mnist-wts/artifacts/input.bin";
    // 注意：Python 端保存的 output_pytorch.bin 已过 softmax，直接是概率
    std::string pytorch_output_path = "tensorrt-8.6/01-mnist-wts/artifacts/output_pytorch.bin";
    ...
    if (fileExists(engine_path)) {
        ...
        engine = buildLenetEngine(builder, config, weightMap);
        assert(engine);
